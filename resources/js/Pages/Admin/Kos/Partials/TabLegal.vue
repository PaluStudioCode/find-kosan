...
const uploadDocument = () => {
    form.post(route('admin.kos.legal-documents.store', props.kos.id), {
        preserveScroll: true,
        onSuccess: () => {
...
<template>
    <div class="space-y-8">
        ...
        <fieldset :disabled="isLocked" :class="{ 'opacity-60': isLocked }">
            <div>
                <div class="bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-900 text-blue-800 dark:text-blue-400 p-4 rounded-lg flex items-start mb-6">
            ...
            <h3 class="text-lg font-semibold border-b dark:border-slate-800 pb-2 mb-4">Unggah Dokumen Baru</h3>
            <form @submit.prevent="uploadDocument" class="bg-gray-50 dark:bg-slate-800/50 p-4 rounded border dark:border-slate-800 grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                <div class="space-y-2">
        ...
        <div>
            <h3 class="text-lg font-semibold border-b dark:border-slate-800 pb-2 mb-4">Daftar Dokumen Anda</h3>
            
            <div v-if="!kos.legal_documents || kos.legal_documents.length === 0" class="text-center py-8 text-gray-500 dark:text-slate-400 border dark:border-slate-800 rounded bg-gray-50 dark:bg-slate-800/50 border-dashed">
                Belum ada dokumen legalitas. Minimal 1 dokumen diperlukan sebelum mengajukan verifikasi.
            </div>

            <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div v-for="doc in kos.legal_documents" :key="doc.id" class="border dark:border-slate-800 rounded-lg p-4 flex justify-between items-center bg-white dark:bg-slate-900 shadow-sm hover:shadow-md transition">
                    <div class="flex items-center space-x-3">
                        <div class="bg-gray-100 dark:bg-slate-800 p-2 rounded text-gray-500 dark:text-slate-400">
                            <FileText class="w-6 h-6" />
                        </div>
                        <div>
                            <p class="font-medium text-gray-900 dark:text-slate-200">{{ doc.document_type }}</p>
                            <p class="text-xs text-gray-500 dark:text-slate-400">Tersimpan secara rahasia</p>
                        </div>
                    </div>
                    <Button variant="outline" size="sm" class="text-red-500 hover:text-red-700 hover:bg-red-50 dark:hover:bg-red-950/30" @click="confirmDocDeletion(doc.id)" :disabled="isLocked">
