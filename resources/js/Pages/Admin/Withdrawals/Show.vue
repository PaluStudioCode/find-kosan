...
<template>
    <AppLayout>
        ...
        <Link :href="route('admin.withdrawals.index')" class="inline-flex items-center text-sm text-gray-500 hover:text-primary dark:text-slate-400 dark:hover:text-blue-400 mb-5 transition-colors"><ChevronLeft class="w-4 h-4 mr-1" /> Kembali ke penarikan</Link>

        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
            <div><h2 class="text-2xl font-bold dark:text-white">Penarikan #{{ withdrawal.id }}</h2><p class="text-gray-500 dark:text-slate-400">{{ withdrawal.owner?.name }} · {{ withdrawal.owner?.email }}</p></div>
