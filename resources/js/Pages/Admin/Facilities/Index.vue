...
<template>
    <AppLayout>
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
            <div>
                <h2 class="text-2xl font-bold text-gray-900 dark:text-white">Master Fasilitas</h2>
                <p class="text-gray-500 dark:text-slate-400 mt-1">Kelola data master fasilitas untuk Kos dan Kamar.</p>
            </div>
            ...
            <CardContent class="p-0 border-0">
                        ...
                        <tbody>
                            <tr v-for="facility in facilities.data" :key="facility.id" class="border-b dark:border-slate-800 dark:hover:bg-slate-800/30 transition-colors">
                                ...
                                <td class="px-4 py-3 text-right flex justify-end gap-2">
                                    <Button variant="outline" size="sm" @click="openEditModal(facility)" class="dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800">
                                        <Edit2 class="w-4 h-4" />
                                    </Button>
                ...
                <div v-if="facilities.links && facilities.links.length > 3" class="p-4 flex gap-1 border-t dark:border-slate-800 bg-gray-50 dark:bg-slate-900 rounded-b-lg">
                    <template v-for="(link, k) in facilities.links" :key="k">
                        ...
                        <button v-else @click="router.get(link.url, filters, {preserveState: true})" class="px-3 py-1 text-sm border dark:border-slate-700 rounded hover:bg-gray-100 dark:hover:bg-slate-700 bg-white dark:bg-slate-800 dark:text-slate-300 transition-colors" :class="{ 'bg-primary text-white border-primary hover:bg-primary dark:bg-blue-600 dark:text-white dark:border-blue-600 dark:hover:bg-blue-700': link.active }" v-html="link.label" />
                    </template>
                </div>
            </CardContent>
        </Card>
        ...
        <Dialog :open="isModalOpen" @update:open="val => isModalOpen = val">
            <DialogContent class="sm:max-w-[425px] dark:bg-slate-900 dark:border-slate-800">
                <DialogHeader>
                    <DialogTitle class="dark:text-white">{{ isEditing ? 'Edit Fasilitas' : 'Tambah Fasilitas Baru' }}</DialogTitle>
                </DialogHeader>
                <form @submit.prevent="submit" class="space-y-4 mt-4">
                    <div class="space-y-2">
                        <Label for="name" class="dark:text-slate-300">Nama Fasilitas</Label>
                        <Input id="name" type="text" v-model="form.name" required class="dark:bg-slate-800 dark:border-slate-700 dark:text-white dark:placeholder-slate-500" />
                        <p v-if="form.errors.name" class="text-sm text-destructive dark:text-red-400">{{ form.errors.name }}</p>
                    </div>

                    <div class="space-y-2">
                        <Label for="type" class="dark:text-slate-300">Tipe</Label>
                        ...
                        <p v-if="form.errors.type" class="text-sm text-destructive dark:text-red-400">{{ form.errors.type }}</p>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <p v-if="form.errors.status" class="text-sm text-destructive dark:text-red-400">{{ form.errors.status }}</p>
                    </div>

                    <div class="mt-6 flex justify-end gap-3">
                        <Button type="button" variant="outline" @click="isModalOpen = false" class="dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800">Batal</Button>
        ...
        <Dialog :open="confirmingFacilityDeletion" @update:open="val => { if(!val) closeDeleteModal(); }">
            <DialogContent class="sm:max-w-[425px] dark:bg-slate-900 dark:border-slate-800">
                ...
                <DialogFooter class="mt-6 flex justify-end gap-3 sm:justify-end">
                    <Button variant="outline" @click="closeDeleteModal" class="dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800">Batal</Button>
